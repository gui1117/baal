...
pub fn amplify_ctrl<I>(input: I, factor: Arc<AtomicPtr<f32>>) -> AmplifyCtrl<I>
...
impl<I> Iterator for AmplifyCtrl<I> where I: Source, I::Item: Sample {
    ...
    fn next(&mut self) -> Option<I::Item> {
        self.input.next().map(|value| value.amplify(unsafe {*self.factor.load(Ordering::Relaxed)}))
    }
