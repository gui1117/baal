//! as part of the amethyst engine, configuration is made in a yaml file.
//!
//! ```yaml
//! ---
//! //TODO complete
...
/// set the volume of sounds effects
/// take effect for future sounds effects only
pub fn effect_set_volume(v: f32) {
    let mut state = STATE.lock().unwrap();
    state.effect_volume = v;
}

/// get the volume of sounds effects
pub fn effect_volume() -> f32 {
...
pub fn effect_play(effect: usize, pos: &[f64;3]) {
    let state = STATE.lock().unwrap();
...
/// take effect for future sounds effects only
pub fn effect_set_distance_model(d: DistanceModel) {
    let mut state = STATE.lock().unwrap();
...
pub fn global_set_volume(v: f32) {
    let mut state = STATE.lock().unwrap();
...
/// init the audio player using the configuration file in yaml
...
impl State {
    fn from_setting(s: Setting,sender: Sender<Msg>,abort_sender: Sender<()>) -> State {
        ...
        debug_assert!({
            for m in &music {
                let snd_file = SndFile::new(m,OpenMode::Read).expect("cannot load a music file");
                let snd_info = snd_file.get_sndinfo();
                assert_eq!(snd_info.samplerate as f64,s.sample_rate);
                assert_eq!(snd_info.channels, s.channels);
            }
            true
        });
